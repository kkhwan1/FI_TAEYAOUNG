/**
 * Excel 파일에서 읽은 BOM 데이터를 DB에 추가하는 스크립트
 * 
...
function parseExcelData(excelRows: ExcelRow[]): ParsedBOMRow[] {
  ...
  for (let i = 0; i < excelRows.length; i++) {
    ...
    const 납품처 = row[0] ? String(row[0]).trim() : '';
    const parent차종 = row[1] ? String(row[1]).trim() : '';
    const parent품번 = row[2] ? String(row[2]).trim() : '';
    const parent품명 = row[3] ? String(row[3]).trim() : '';
    const 구매처 = row[8] ? String(row[8]).trim() : '';
    const child차종 = row[9] ? String(row[9]).trim() : '';
    const child품번 = row[10] ? String(row[10]).trim() : '';
    const child품명 = row[11] ? String(row[11]).trim() : '';
    const usage = row[12] ? String(row[12]).trim() : '';
    const 비고 = row[16] ? String(row[16]).trim() : '';

    // Parent 정보 업데이트 (납품처와 품번이 있으면)
    if (납품처 && parent품번) {
      currentParent = {
        납품처,
        차종: parent차종,
...
async function importBOMData(bomRows: ParsedBOMRow[]) {
  ...
  console.log(`준비된 BOM 데이터: ${bomDataToInsert.length}개`);
