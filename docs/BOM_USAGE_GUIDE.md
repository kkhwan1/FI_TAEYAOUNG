...
## 18. BOM 데이터 검증 현황 (2025-12-01)

### 18.1 Excel → DB 동기화 검증 결과

**검증일**: 2025-12-01

#### 검증 대상
- **Excel 파일**: `(추가)BOM 종합 - ERP (1) copy - 복사본.xlsx`
- **시트 구성**: 대우당진, 대우포승, 풍기서산, 호원오토, 인알파코리아
...
#### 템플릿 데이터 처리
다음 공급업체는 템플릿 데이터로 처리되어 DB에 추가되지 않습니다:
- `대우포승 사급`
- `하드웨어`
- `현대제철`

#### 회사명 매핑
- `코리아신예(무상사급)` → `코리아신예` (DB 등록명으로 자동 변환)

### 18.2 BOM 레코드 구성

| 유형 | 개수 | 설명 |
|------|------|------|
| 일반 BOM 관계 | 369개 | parent ≠ child |
| Self-referencing | 164개 | parent = child (공급업체 지정용) |
| 비활성 (테스트) | 45개 | TEST_COMPANY |
| 비활성 (생산실적) | 42개 | 풍기광주, 다인, 대우공업 등 |

### 18.3 검증 스크립트

검증에 사용된 스크립트: `scripts/compare-bom-excel-db.js`

```javascript
// 핵심 설정
const SKIP_SUPPLIERS = ['대우포승 사급', '하드웨어', '현대제철'];
const COMPANY_NAME_MAP = { '코리아신예(무상사급)': '코리아신예' };
```

### 18.4 관련 문서

- [BOM 동기화 최종 검증 보고서](./BOM_동기화_최종_검증_보고서_2025-12-01.md)
