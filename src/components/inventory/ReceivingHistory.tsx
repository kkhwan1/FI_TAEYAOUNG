  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // 금일 데이터만 필터링
        const todayData = (Array.isArray(transactionsData) ? transactionsData : []).filter((t: ReceivingTransaction) => {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-700 text-xs text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                ...
                <th className="px-2 py-2 text-left font-semibold" style={{width: '120px'}}>공급업체</th>
