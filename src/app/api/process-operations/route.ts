...
export async function GET(request: NextRequest) {
  try {
    ...
    const operations: ProcessOperationWithItems[] = (data || []).map((op: any) => ({
      ...
      input_quantity: Number(op.input_quantity || 0),
      output_quantity: Number(op.output_quantity || 0),
      efficiency: op.efficiency ? Number(op.efficiency) : undefined,
      operator_id: op.operator_id ?? undefined,
      started_at: op.started_at ?? undefined,
      ...
      // LOT tracking fields
      lot_number: op.lot_number ?? undefined,
      parent_lot_number: op.parent_lot_number ?? undefined,
      child_lot_number: op.child_lot_number ?? undefined,
      // Chain management fields
      chain_id: op.chain_id ?? undefined,
      chain_sequence: op.chain_sequence ?? undefined,
      parent_operation_id: op.parent_operation_id ?? undefined,
      auto_next_operation: op.auto_next_operation ?? undefined,
      next_operation_type: op.next_operation_type ?? undefined,
      // Quality control fields
      quality_status: op.quality_status ?? undefined,
      scrap_quantity: op.scrap_quantity ? Number(op.scrap_quantity) : undefined,
      scheduled_date: op.scheduled_date ?? undefined,
      input_item: op.input_item ? {
        item_id: op.input_item.item_id,
        item_name: op.input_item.item_name || '-',
        item_code: op.input_item.item_code || '-',
        current_stock: Number(op.input_item.current_stock || 0),
        unit: op.input_item.unit ?? undefined,
        spec: op.input_item.spec ?? undefined,
      } : {
      ...
      output_item: op.output_item ? {
        ...
        current_stock: Number(op.output_item.current_stock || 0),
        unit: op.output_item.unit ?? undefined,
        spec: op.output_item.spec ?? undefined,
      } : {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const currentStock = parseFloat(String(inputItem.current_stock));
        ...
        if (!coilSpecError && coilSpec) {
          ...
          const { data: coilProcess, error: coilProcessError } = await supabase
            ...
            .insert({
              ...
              status: 'PENDING', // New operations are always PENDING
              operator_id: operatorId,
    ...
    const operation: ProcessOperationWithItems = {
      ...
      operation_type: newOperation.operation_type as OperationType,
      input_item_id: newOperation.input_item_id,
      output_item_id: newOperation.output_item_id,
      input_quantity: Number(newOperation.input_quantity),
      output_quantity: Number(newOperation.output_quantity),
      efficiency: newOperation.efficiency ? Number(newOperation.efficiency) : undefined,
      operator_id: newOperation.operator_id ?? undefined,
      started_at: newOperation.started_at ?? undefined,
      completed_at: newOperation.completed_at ?? undefined,
      ...
      // LOT tracking fields
      lot_number: newOperation.lot_number ?? undefined,
      parent_lot_number: newOperation.parent_lot_number ?? undefined,
      child_lot_number: newOperation.child_lot_number ?? undefined,
      // Chain management fields
      chain_id: newOperation.chain_id ?? undefined,
      chain_sequence: newOperation.chain_sequence ?? undefined,
      parent_operation_id: newOperation.parent_operation_id ?? undefined,
      auto_next_operation: newOperation.auto_next_operation ?? undefined,
      next_operation_type: newOperation.next_operation_type ?? undefined,
      // Quality control fields
      quality_status: newOperation.quality_status ?? undefined,
      scrap_quantity: newOperation.scrap_quantity ? Number(newOperation.scrap_quantity) : undefined,
      scheduled_date: newOperation.scheduled_date ?? undefined,
      ...
      input_item: {
        ...
        current_stock: Number(newOperation.input_item.current_stock),
        unit: newOperation.input_item.unit ?? undefined,
        spec: newOperation.input_item.spec ?? undefined,
      },
      output_item: {
        ...
        current_stock: Number(newOperation.output_item.current_stock),
        unit: newOperation.output_item.unit ?? undefined,
        spec: newOperation.output_item.spec ?? undefined,
      },
    };
